...
Serverless Warmer reconfigures serverless.yml function events to enable warmer events one minute prior to the event, eliminating the need for idle times in between function calls

This sourcecode includes a simple, working serverless directory and the adaptations to the original serverless-plugin-warmup

Below is juanjoDiaz's "serverless-plugin-warmup" steps on how to install and use the original warmer
The original module can be found here: https://github.com/juanjoDiaz/serverless-plugin-warmup#options

This variation currently allows for rates and exact times in AWS CRON syntax by extracting CRON schedules from the 'functions' section of the user's unique serverless.yml file

Events are then reprogrammed to run one minute before the original CRON before being repackaged with the warmer and sent to AWS CloudWatch

All adaptations occur in config.js


# Serverless WarmUp Plugin by juanjoDiaz ♨
